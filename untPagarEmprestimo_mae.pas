...
type
  TfrmPagarEmprestimo_mae = class(TForm)
    GroupBox1: TGroupBox;
    txt_codigo: TEdit;
    Label8: TLabel;
    txt_data: TDateEdit;
...
var
  frmPagarEmprestimo_mae: TfrmPagarEmprestimo_mae;

implementation

uses unt_modulo, untEmprestimosPagar_mae;

{$R *.dfm}

procedure TfrmPagarEmprestimo_mae.btnGravarClick(Sender: TObject);
var
...
begin
  ...
  saldoPagar := frmEmprestimosPagar_mae.qryEmprestimo.fieldbyname('SALDO_A_PAGAR').AsFloat;
  ...
  QRY_MAX_CODIGO.SQL.Add('SELECT MAX(PG_CODIGO) AS CODIGO FROM PAGTO_EMPRESTIMOS_A_PAGAR_MAE');
  QRY_MAX_CODIGO.Open;
  ...
  qry_trans_local.SQL.Add('INSERT INTO PAGTO_EMPRESTIMOS_A_PAGAR_MAE (PG_CODIGO, CODIGO, PG_VALOR, PG_DATA, PG_OBS, TP_CODIGO)  '+
  ...
  qryRecebido.SQL.Add('SELECT EP.*, EP.TP_CODIGO ||'' - ''|| TP.TP_DESCRICAO AS DESCRICAO FROM PAGTO_EMPRESTIMOS_A_PAGAR_MAE EP '+
  ...
  qryTotal.Close;
  qryTotal.SQL.Clear;
  qryTotal.SQL.Add('SELECT SUM(PG_VALOR) AS TOTAL FROM PAGTO_EMPRESTIMOS_A_PAGAR_MAE WHERE CODIGO=:CODIGO');
  ...
  qry_trans_local.Close;
  qry_trans_local.SQL.Clear;
  qry_trans_local.SQL.Add('UPDATE EMPRESTIMOS_A_PAGAR_MAE SET VALOR_PAGO=:VALOR_PAGO, SALDO_A_PAGAR=:SALDO_A_PAGAR '+
                          ' WHERE CODIGO=:CODIGO                                                                   ');
  qry_trans_local.ParamByName('CODIGO').AsString     := txt_codigo.Text;
  qry_trans_local.ParamByName('VALOR_PAGO').AsFLOAT  := qryTotal.fieldbyname('TOTAL').AsFloat;
  qry_trans_local.ParamByName('SALDO_A_PAGAR').AsFLOAT := frmEmprestimosPagar_mae.qryEmprestimo.fieldbyname('VALOR_A_PAGAR').AsFloat - qryTotal.fieldbyname('TOTAL').AsFloat;
  ...
  data1 := Copy(frmEmprestimosPagar_mae.txt_data_i.Text,4,3) + Copy(frmEmprestimosPagar_mae.txt_data_i.Text,1,3) + Copy(frmEmprestimosPagar_mae.txt_data_i.Text,7,4) + ' 00:00:00';
  data2 := Copy(frmEmprestimosPagar_mae.txt_data_f.Text,4,3) + Copy(frmEmprestimosPagar_mae.txt_data_f.Text,1,3) + Copy(frmEmprestimosPagar_mae.txt_data_f.Text,7,4) + ' 23:59:59';

   if (frmEmprestimosPagar_mae.txt_data_i.Text <> '  /  /    ') and (frmEmprestimosPagar_mae.txt_data_f.Text <> '  /  /    ') then
     dta := ' AND DATA BETWEEN '+ #39 + data1 + #39 + ' AND '+ #39 + data2 + #39
   else
     dta := '';

   if frmEmprestimosPagar_mae.rdg_opcao.ItemIndex = 0 then
      opcao := ' AND SALDO_A_PAGAR = 0'
   else if frmEmprestimosPagar_mae.rdg_opcao.ItemIndex = 1 then
      opcao := ' AND SALDO_A_PAGAR > 0'
   else
      opcao := '';

  frmEmprestimosPagar_mae.qryEmprestimo.Close;
  frmEmprestimosPagar_mae.qryEmprestimo.SQL.Clear;
  frmEmprestimosPagar_mae.qryEmprestimo.SQL.Add('SELECT EP.*, TP.tp_codigo || '' - '' || TP.tp_descricao AS DESCRICAO  FROM EMPRESTIMOS_A_PAGAR_MAE EP  '+
                                            ' INNER JOIN TIPO_PAGAMENTO_MAE TP ON EP.TP_CODIGO = TP.TP_CODIGO                                           '+
                                            ' WHERE CODIGO = CODIGO ' + dta + opcao + ' ORDER BY CODIGO                                                 ');
  frmEmprestimosPagar_mae.qryEmprestimo.Open;

  frmEmprestimosPagar_mae.qryTotal.Close;
  frmEmprestimosPagar_mae.qryTotal.SQL.Clear;
  frmEmprestimosPagar_mae.qryTotal.SQL.Add('SELECT SUM(VALOR_A_PAGAR) AS TOTAL_A_PAGAR,                         '+
                                       ' SUM(VALOR_PAGO) AS TOTAL_PAGO,                                         '+
                                       ' SUM(SALDO_A_PAGAR) AS TOTAL_SALDO_A_PAGAR FROM EMPRESTIMOS_A_PAGAR_MAE '+
                                       ' WHERE CODIGO = CODIGO ' + dta + opcao                                   );

  frmEmprestimosPagar_mae.qryTotal.Open;

  frmEmprestimosPagar_mae.txt_total_a_pagar.Value := frmEmprestimosPagar_mae.qryTotal.fieldbyname('TOTAL_A_PAGAR').AsFloat;
  frmEmprestimosPagar_mae.txt_total_pago.Value := frmEmprestimosPagar_mae.qryTotal.fieldbyname('TOTAL_PAGO').AsFloat;
  frmEmprestimosPagar_mae.txt_total_saldo_a_pagar.Value := frmEmprestimosPagar_mae.qryTotal.fieldbyname('TOTAL_SALDO_A_PAGAR').AsFloat;
...
procedure TfrmPagarEmprestimo_mae.DBGrid1DrawColumnCell(Sender: TObject;
...
procedure TfrmPagarEmprestimo_mae.txt_dataEnter(Sender: TObject);
begin
  TXT_dATA.Color := $00F5EEDE;
end;

procedure TfrmPagarEmprestimo_mae.txt_dataExit(Sender: TObject);
begin
  txt_data.Color := clwindow;
end;

procedure TfrmPagarEmprestimo_mae.txt_dataKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    txt_valor.SetFocus;
...
procedure TfrmPagarEmprestimo_mae.txt_ValorEnter(Sender: TObject);
begin
  txt_Valor.Color := $00F5EEDE;
end;

procedure TfrmPagarEmprestimo_mae.txt_ValorExit(Sender: TObject);
begin
  txt_valor.Color := clwindow;
end;

procedure TfrmPagarEmprestimo_mae.txt_ValorKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
...
procedure TfrmPagarEmprestimo_mae.txt_obsEnter(Sender: TObject);
begin
  txt_obs.Color := $00F5EEDE;
end;

procedure TfrmPagarEmprestimo_mae.txt_obsExit(Sender: TObject);
begin
  txt_obs.Color := clwindow;
end;

procedure TfrmPagarEmprestimo_mae.txt_obsKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    btnGravar.SetFocus;
...
procedure TfrmPagarEmprestimo_mae.FormShow(Sender: TObject);
begin
...
procedure TfrmPagarEmprestimo_mae.cboFormaPgtoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    txt_obs.SetFocus;
...
procedure TfrmPagarEmprestimo_mae.cboFormaPgtoEnter(Sender: TObject);
begin
  cboFormapgto.color := $00F5EEDE;
end;

procedure TfrmPagarEmprestimo_mae.cboFormaPgtoExit(Sender: TObject);
begin
  cboformapgto.color := clwindow;
end;

procedure TfrmPagarEmprestimo_mae.btnPesquisarClick(Sender: TObject);
var
...
begin
  ...
  qryTotal.Close;
  qryTotal.SQL.Clear;
  qryTotal.SQL.Add('SELECT SUM(PG_VALOR) AS TOTAL FROM PAGTO_EMPRESTIMOS_A_PAGAR_MAE EP INNER JOIN TIPO_PAGAMENTO_MAE TP ON EP.TP_CODIGO = TP.TP_CODIGO ');
...
procedure TfrmPagarEmprestimo_mae.DBGrid1TitleClick(Column: TColumn);
var
...
begin
  ...
  qryRecebido.SQL.Add('SELECT EP.PG_CODIGO, EP.PG_DATA, EP.PG_VALOR, EP.PG_OBS, EP.TP_CODIGO, EP.CODIGO,            '+
                      ' EP.TP_CODIGO ||'' - ''|| TP.TP_DESCRICAO AS DESCRICAO FROM PAGTO_EMPRESTIMOS_A_PAGAR_MAE EP '+
                      ' INNER JOIN TIPO_PAGAMENTO_MAE TP ON EP.TP_CODIGO = TP.TP_CODIGO                             '+
                      ' WHERE 1=1 ' + dta + obs + tipo_pagamento                                                     +
                      ' ORDER BY ' + campo                                                                           );
  qryRecebido.Open;
