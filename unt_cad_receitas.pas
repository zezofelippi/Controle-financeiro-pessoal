...
begin
  ...
  QRY_MAX_CODIGO.SQL.Add('SELECT MAX(REC_CODIGO) AS CODIGO FROM CAD_RECEITAS');
  QRY_MAX_CODIGO.Open;
  ...
  begin
    QRY_MAX_CODIGO.Close;
    QRY_MAX_CODIGO.SQL.Clear;
    QRY_MAX_CODIGO.SQL.Add('SELECT MAX(COD_MES) AS COD_MES FROM CAD_RECEITAS');
    QRY_MAX_CODIGO.Open;
  ...
  qry_receita.SQL.Add('INSERT INTO CAD_RECEITAS (REC_CODIGO, CODIGO, VALOR, DATA, COD_MES, OBS, TP_CODIGO)  '+
  ...
  qry_pesquisa.Close;
  qry_pesquisa.SQL.Clear;
  qry_pesquisa.SQL.Add('SELECT COD_MES FROM CAD_DESPESA WHERE COD_MES=:COD_MES');
  ...
  begin
    ...
    qry_pesquisa.SQL.Add('SELECT EXTRACT(month FROM DATA) AS MES, EXTRACT(YEAR FROM DATA) AS ANO, COD_MES, CODIGO  '+
                         'FROM CAD_DESPESA WHERE EXTRACT(month FROM DATA) =:MES                                    '+
                         'AND EXTRACT(YEAR FROM DATA) =:ANO                                                        ');
    qry_pesquisa.ParamByName('MES').AsString := mes;
    qry_pesquisa.ParamByName('ANO').AsString := ano;
    qry_pesquisa.Open;
    ...
    qry_receita.Close;
    qry_receita.SQL.Clear;
    qry_receita.SQL.Add('UPDATE CAD_DESPESA SET COD_MES=:COD_MES WHERE CODIGO=:CODIGO');
...
begin
  ...
  qryPesquisa.SQL.Add(' SELECT T.RECEITA, C.VALOR, C.DATA, C.OBS, C.TP_CODIGO ||'' - ''|| TP.TP_DESCRICAO AS DESCRICAO  '+
                      ...
                      ' INNER JOIN TIPO_PAGAMENTO TP ON TP.TP_CODIGO = C.TP_CODIGO WHERE  1=1                           '+
  ...
  qryTotal.Close;
  qryTotal.SQL.Clear;
  qryTotal.SQL.Add('SELECT SUM(C.VALOR) AS TOTAL FROM CAD_RECEITAS C, TIPO_RECEITA T WHERE  C.CODIGO = T.CODIGO ' + tipo_receita + dta + receita + obsPesquisa);
  qryTotal.Open;
...
procedure Tfrm_cad_receitas.txt_data_iEnter(Sender: TObject);
begin
  txt_Data_i.Color := $00F5EEDE;
end;

procedure Tfrm_cad_receitas.txt_data_iExit(Sender: TObject);
begin
  txt_Data_i.Color := clwindow;
end;

procedure Tfrm_cad_receitas.txt_data_iKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    txt_data_f.SetFocus;
...
procedure Tfrm_cad_receitas.txt_data_fEnter(Sender: TObject);
begin
  txt_data_f.Color := $00F5EEDE;
end;

procedure Tfrm_cad_receitas.txt_data_fExit(Sender: TObject);
begin
  txt_data_f.Color := clwindow;
end;

procedure Tfrm_cad_receitas.txt_data_fKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
...
procedure Tfrm_cad_receitas.cbo_receita_pesquisaEnter(Sender: TObject);
begin
  cbo_receita_pesquisa.Color := $00F5EEDE;
end;

procedure Tfrm_cad_receitas.cbo_receita_pesquisaExit(Sender: TObject);
begin
  cbo_receita_pesquisa.Color := clwindow;
end;

procedure Tfrm_cad_receitas.cbo_receita_pesquisaKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
...
procedure Tfrm_cad_receitas.cboFormaPgtoKeyDown(Sender: TObject;
...
procedure Tfrm_cad_receitas.cboFormaPgtoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key=#13 then
    btnGravar.SetFocus;
...
procedure Tfrm_cad_receitas.cboFormaPgtoEnter(Sender: TObject);
begin
  cboFormaPgto.color := $00F5EEDE;
end;

procedure Tfrm_cad_receitas.cboFormaPgtoExit(Sender: TObject);
begin
  cboformaPgto.color := clwindow;
end;

procedure Tfrm_cad_receitas.cboFormaPgtoPesqEnter(Sender: TObject);
begin
  cboFormapgtopesq.color := $00F5EEDE;
end;

procedure Tfrm_cad_receitas.cboFormaPgtoPesqExit(Sender: TObject);
begin
  cboFormapgtopesq.color := clwindow;
end;

procedure Tfrm_cad_receitas.cboFormaPgtoPesqKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
...
procedure Tfrm_cad_receitas.cboFormaPgtoPesqKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  IF KEY =VK_back then
