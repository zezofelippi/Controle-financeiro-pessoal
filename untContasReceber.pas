...
uses unt_modulo, untReceberEmprestimo, unt_funcoes;

{$R *.dfm}

procedure TfrmContasReceber.txt_dataEnter(Sender: TObject);
begin
  txt_data.Color :=$00F5EEDE;
end;

procedure TfrmContasReceber.txt_dataExit(Sender: TObject);
begin
  txt_data.Color := clwindow;
end;

procedure TfrmContasReceber.txt_dataKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    txt_valor.SetFocus;
...
procedure TfrmContasReceber.txt_ValorEnter(Sender: TObject);
begin
  txt_valor.Color := $00F5EEDE;
end;

procedure TfrmContasReceber.txt_ValorExit(Sender: TObject);
begin
  txt_valor.Color := clwindow;
end;

procedure TfrmContasReceber.txt_ValorKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
...
procedure TfrmContasReceber.txt_obsEnter(Sender: TObject);
begin
  txt_obs.Color :=$00F5EEDE;
end;

procedure TfrmContasReceber.txt_obsExit(Sender: TObject);
begin
  txt_obs.Color := clwindow;
end;

procedure TfrmContasReceber.txt_obsKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    btnGravar.SetFocus;
...
begin
  ...
  begin
    ...
    QRY_MAX_CODIGO.SQL.Add('SELECT MAX(CODIGO) AS CODIGO FROM EMPRESTIMOS_A_RECEBER');
    QRY_MAX_CODIGO.Open;
    ...
    qry_trans_local.SQL.Add('INSERT INTO EMPRESTIMOS_A_RECEBER (CODIGO, VALOR_EMPRESTIMO, VALOR_RECEBIDO, SALDO_A_RECEBER, DATA, OBS, TP_CODIGO)  '+
  ...
  begin
    qry_trans_local.Close;
    qry_trans_local.SQL.Clear;
    qry_trans_local.SQL.Add('UPDATE EMPRESTIMOS_A_RECEBER SET VALOR_EMPRESTIMO=:VALOR_EMPRESTIMO, VALOR_RECEBIDO=:VALOR_RECEBIDO, '+
...
procedure TfrmContasReceber.txt_data_iEnter(Sender: TObject);
begin
  txt_data_i.Color :=$00F5EEDE;
end;

procedure TfrmContasReceber.txt_data_iExit(Sender: TObject);
begin
  txt_data_i.Color := clwindow;
end;

procedure TfrmContasReceber.txt_data_iKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    txt_Data_f.SetFocus;
...
procedure TfrmContasReceber.txt_data_fEnter(Sender: TObject);
begin
  txt_Data_f.Color :=$00F5EEDE;
end;

procedure TfrmContasReceber.txt_data_fExit(Sender: TObject);
begin
  txt_data_f.Color := clwindow;
end;

procedure TfrmContasReceber.txt_data_fKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
...
begin
  ...
  qryTotal.SQL.Add('SELECT SUM(VALOR_EMPRESTIMO) AS TOTAL_EMPRESTIMO,                      '+
                   ...
                   ' SUM(SALDO_A_RECEBER) AS TOTAL_A_RECEBER FROM EMPRESTIMOS_A_RECEBER ER '+
                   ' INNER JOIN tipo_pagamento TP ON ER.tp_codigo = TP.tp_codigo           '+
...
procedure TfrmContasReceber.Receber1Click(Sender: TObject);
begin

  Application.CreateForm(TfrmReceberEmprestimo, frmReceberEmprestimo);

  frmReceberEmprestimo.txt_codigo.Text :=  qryEmprestimo.fieldbyname('CODIGO').AsString;

  frmReceberEmprestimo.qryRecebido.Close;
  frmReceberEmprestimo.qryRecebido.SQL.Clear;
  frmReceberEmprestimo.qryRecebido.SQL.Add('SELECT ER.*, TP.tp_codigo || '' - '' || TP.tp_descricao AS DESCRICAO FROM PAGTO_EMPRESTIMOS_A_RECEBER ER '+
                                           ' INNER JOIN TIPO_PAGAMENTO TP ON ER.TP_CODIGO = TP.TP_CODIGO                                             '+
                                           ' WHERE CODIGO=:CODIGO ORDER BY PG_CODIGO                                                                 ');
  frmReceberEmprestimo.qryRecebido.ParamByName('CODIGO').AsString := frmReceberEmprestimo.txt_codigo.Text;
  frmReceberEmprestimo.qryRecebido.Open;

  frmReceberEmprestimo.qryTotal.Close;
  frmReceberEmprestimo.qryTotal.SQL.Clear;
  frmReceberEmprestimo.qryTotal.SQL.Add('SELECT SUM(PG_VALOR) AS TOTAL FROM PAGTO_EMPRESTIMOS_A_RECEBER WHERE CODIGO=:CODIGO');
  frmReceberEmprestimo.qryTotal.ParamByName('CODIGO').AsString := frmReceberEmprestimo.txt_codigo.Text;
  frmReceberEmprestimo.qryTotal.Open;

  frmReceberEmprestimo.txt_total_recebido.Value := frmReceberEmprestimo.qryTotal.fieldbyname('TOTAL').AsFloat;

  frmReceberEmprestimo.show;

end;

procedure TfrmContasReceber.TXT_OBS_pesqEnter(Sender: TObject);
begin
  txt_obs_pesq.Color := $00F5EEDE;
end;

procedure TfrmContasReceber.TXT_OBS_pesqExit(Sender: TObject);
begin
  txt_obs_pesq.Color := clwindow;
end;

procedure TfrmContasReceber.TXT_OBS_pesqKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
...
procedure TfrmContasReceber.cboFormaPgtoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    txt_obs.SetFocus;
...
procedure TfrmContasReceber.btn1Click(Sender: TObject);
begin
  Application.CreateForm(TfrmReceberEmprestimo, frmReceberEmprestimo);
  frmReceberEmprestimo.show;
end;
