...
type
  TfrmReceberEmprestimo = class(TForm)
    GroupBox1: TGroupBox;
    txt_codigo: TEdit;
    Label8: TLabel;
    txt_data: TDateEdit;
...
var
  frmReceberEmprestimo: TfrmReceberEmprestimo;

implementation

uses unt_modulo, untContasReceber;

{$R *.dfm}

procedure TfrmReceberEmprestimo.btnGravarClick(Sender: TObject);
var
...
begin
  ...
  saldoReceber := frmContasReceber.qryEmprestimo.fieldbyname('SALDO_A_RECEBER').AsFloat;
  ...
  QRY_MAX_CODIGO.SQL.Add('SELECT MAX(PG_CODIGO) AS CODIGO FROM PAGTO_EMPRESTIMOS_A_RECEBER');
  QRY_MAX_CODIGO.Open;
  cod_recebimento := QRY_MAX_CODIGO.FieldByName('CODIGO').AsInteger +1;

  qry_trans_local.Close;
  qry_trans_local.SQL.Clear;
  qry_trans_local.SQL.Add('INSERT INTO PAGTO_EMPRESTIMOS_A_RECEBER (PG_CODIGO, CODIGO, PG_VALOR, PG_DATA, PG_OBS, TP_CODIGO)  '+
  ...
  qryRecebido.SQL.Add('SELECT ER.*, TP.tp_codigo || '' - '' || TP.tp_descricao AS DESCRICAO FROM PAGTO_EMPRESTIMOS_A_RECEBER ER '+
  ...
  qryTotal.Close;
  qryTotal.SQL.Clear;
  qryTotal.SQL.Add('SELECT SUM(PG_VALOR) AS TOTAL FROM PAGTO_EMPRESTIMOS_A_RECEBER WHERE CODIGO=:CODIGO');
  ...
  qry_trans_local.Close;
  qry_trans_local.SQL.Clear;
  qry_trans_local.SQL.Add('UPDATE EMPRESTIMOS_A_RECEBER SET VALOR_RECEBIDO=:VALOR_RECEBIDO, SALDO_A_RECEBER=:SALDO_A_RECEBER '+
                          ' WHERE CODIGO=:CODIGO                                                                             ');
  qry_trans_local.ParamByName('CODIGO').AsString         := txt_codigo.Text;
  qry_trans_local.ParamByName('VALOR_RECEBIDO').AsFLOAT  := qryTotal.fieldbyname('TOTAL').AsFloat;
  qry_trans_local.ParamByName('SALDO_A_RECEBER').AsFLOAT := frmContasReceber.qryEmprestimo.fieldbyname('VALOR_EMPRESTIMO').AsFloat - qryTotal.fieldbyname('TOTAL').AsFloat;
  ...
  data1 := Copy(frmContasReceber.txt_data_i.Text,4,3) + Copy(frmContasReceber.txt_data_i.Text,1,3) + Copy(frmContasReceber.txt_data_i.Text,7,4) + ' 00:00:00';
  data2 := Copy(frmContasReceber.txt_data_f.Text,4,3) + Copy(frmContasReceber.txt_data_f.Text,1,3) + Copy(frmContasReceber.txt_data_f.Text,7,4) + ' 23:59:59';

  if (frmContasReceber.txt_data_i.Text <> '  /  /    ') and (frmContasReceber.txt_data_f.Text <> '  /  /    ') then
    dta := '  AND DATA BETWEEN '+ #39 + data1 + #39 + ' AND '+ #39 + data2 + #39
  else
    dta := '';


  if frmContasReceber.rdg_opcao.ItemIndex = 0 then
     opcao := ' AND SALDO_A_RECEBER = 0'
  else if frmContasReceber.rdg_opcao.ItemIndex = 1 then
     opcao := ' AND SALDO_A_RECEBER > 0'
  else
     opcao := '';

  frmContasReceber.qryEmprestimo.Close;
  frmContasReceber.qryEmprestimo.SQL.Clear;
  frmContasReceber.qryEmprestimo.SQL.Add('SELECT ER.*, TP.tp_codigo || '' - '' || TP.tp_descricao AS DESCRICAO '+
           'FROM EMPRESTIMOS_A_RECEBER ER                                                                      '+
           'INNER JOIN tipo_pagamento TP ON ER.tp_codigo = TP.tp_codigo                                        '+
           'WHERE 1=1 ' + dta + opcao                                                                           +
           'ORDER BY CODIGO                                                                                    ');
  frmContasReceber.qryEmprestimo.Open;

  frmContasReceber.qryTotal.Close;
  frmContasReceber.qryTotal.SQL.Clear;
  frmContasReceber.qryTotal.SQL.Add('SELECT SUM(VALOR_EMPRESTIMO) AS TOTAL_EMPRESTIMO,                   '+
                                    ' SUM(VALOR_RECEBIDO) AS TOTAL_RECEBIDO,                             '+
                                    ' SUM(SALDO_A_RECEBER) AS TOTAL_A_RECEBER FROM EMPRESTIMOS_A_RECEBER '+
                                    ' WHERE CODIGO = CODIGO ' + dta + opcao                              );
  frmContasReceber.qryTotal.Open;

  frmContasReceber.txt_total_emprestado.Value := frmContasReceber.qryTotal.fieldbyname('TOTAL_EMPRESTIMO').AsFloat;
  frmContasReceber.txt_total_recebido.Value := frmContasReceber.qryTotal.fieldbyname('TOTAL_RECEBIDO').AsFloat;
  frmContasReceber.txt_total_a_receber.Value := frmContasReceber.qryTotal.fieldbyname('TOTAL_A_RECEBER').AsFloat;
...
procedure TfrmReceberEmprestimo.DBGrid1DrawColumnCell(Sender: TObject;
...
procedure TfrmReceberEmprestimo.txt_dataEnter(Sender: TObject);
begin
  TXT_dATA.Color := $00F5EEDE;
end;

procedure TfrmReceberEmprestimo.txt_dataExit(Sender: TObject);
begin
  txt_data.Color := clwindow;
end;

procedure TfrmReceberEmprestimo.txt_dataKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    txt_valor.SetFocus;
...
procedure TfrmReceberEmprestimo.txt_ValorEnter(Sender: TObject);
begin
  txt_Valor.Color := $00F5EEDE;
end;

procedure TfrmReceberEmprestimo.txt_ValorExit(Sender: TObject);
begin
  txt_valor.Color := clwindow;
end;

procedure TfrmReceberEmprestimo.txt_ValorKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
...
procedure TfrmReceberEmprestimo.txt_obsEnter(Sender: TObject);
begin
  txt_obs.Color := $00F5EEDE;
end;

procedure TfrmReceberEmprestimo.txt_obsExit(Sender: TObject);
begin
  txt_obs.Color := clwindow;
end;

procedure TfrmReceberEmprestimo.txt_obsKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    btnGravar.SetFocus;
...
procedure TfrmReceberEmprestimo.FormShow(Sender: TObject);
begin
  qryFormaPgto.Close;
...
procedure TfrmReceberEmprestimo.cboFormaPgtoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    txt_obs.SetFocus;
...
procedure TfrmReceberEmprestimo.cboFormaPgtoEnter(Sender: TObject);
begin
  cboFormaPgto.color := $00F5EEDE;
end;

procedure TfrmReceberEmprestimo.cboFormaPgtoExit(Sender: TObject);
begin
  cboFormaPgto.color := clwindow;
end;

procedure TfrmReceberEmprestimo.btnPesquisarClick(Sender: TObject);
var
...
begin
  ...
  qryRecebido.SQL.Add('SELECT ER.PG_CODIGO, ER.PG_DATA, ER.PG_VALOR, ER.PG_OBS, ER.CODIGO, TP.TP_CODIGO, ER.CODIGO,  '+
                      ...
                      ' FROM PAGTO_EMPRESTIMOS_A_RECEBER ER                                                          '+
                      ' INNER JOIN TIPO_PAGAMENTO TP ON ER.TP_CODIGO = TP.TP_CODIGO                                  '+
  ...
  qryTotal.Close;
  qryTotal.SQL.Clear;
  qryTotal.SQL.Add('SELECT SUM(PG_VALOR) AS TOTAL FROM PAGTO_EMPRESTIMOS_A_RECEBER ER '+
                   'INNER JOIN TIPO_PAGAMENTO TP ON ER.TP_CODIGO = TP.TP_CODIGO       '+
                   'WHERE 1=1 ' + dta + obs + tipo_pagamento                           );
  qryTotal.Open;

  txt_total_recebido.Value := frmReceberEmprestimo.qryTotal.fieldbyname('TOTAL').AsFloat;
end;

procedure TfrmReceberEmprestimo.DBGrid1TitleClick(Column: TColumn);
var
