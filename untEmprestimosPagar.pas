...
type
  TfrmEmprestimosPagar = class(TForm)
    GroupBox1: TGroupBox;
    Label3: TLabel;
...
var
  frmEmprestimosPagar: TfrmEmprestimosPagar;

implementation

uses unt_modulo, untPagarEmprestimo;

{$R *.dfm}

procedure TfrmEmprestimosPagar.txt_dataEnter(Sender: TObject);
begin
  txt_data.Color :=$00F5EEDE;
end;

procedure TfrmEmprestimosPagar.txt_dataExit(Sender: TObject);
begin
  txt_data.Color := clwindow;
end;

procedure TfrmEmprestimosPagar.txt_dataKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    txt_valor.SetFocus;
...
procedure TfrmEmprestimosPagar.txt_ValorEnter(Sender: TObject);
begin
  txt_valor.Color := $00F5EEDE;
end;

procedure TfrmEmprestimosPagar.txt_ValorExit(Sender: TObject);
begin
  txt_valor.Color := clwindow;
end;

procedure TfrmEmprestimosPagar.txt_ValorKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
...
procedure TfrmEmprestimosPagar.txt_obsEnter(Sender: TObject);
begin
  txt_obs.Color :=$00F5EEDE;
end;

procedure TfrmEmprestimosPagar.txt_obsExit(Sender: TObject);
begin
  txt_obs.Color := clwindow;
end;

procedure TfrmEmprestimosPagar.txt_obsKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    btnGravar.SetFocus;
...
procedure TfrmEmprestimosPagar.btnGravarClick(Sender: TObject);
var
...
begin
  ...
  begin
    ...
    QRY_MAX_CODIGO.SQL.Add('SELECT MAX(CODIGO) AS CODIGO FROM EMPRESTIMOS_A_PAGAR');
    QRY_MAX_CODIGO.Open;
    cod_emprestimo := QRY_MAX_CODIGO.FieldByName('CODIGO').AsInteger +1;

    qry_trans_local.Close;
    qry_trans_local.SQL.Clear;
    qry_trans_local.SQL.Add('INSERT INTO EMPRESTIMOS_A_PAGAR (CODIGO, VALOR_A_PAGAR, VALOR_PAGO, SALDO_A_PAGAR, DATA, OBS, TP_CODIGO)  '+
  ...
  begin
    qry_trans_local.Close;
    qry_trans_local.SQL.Clear;
    qry_trans_local.SQL.Add('UPDATE EMPRESTIMOS_A_PAGAR SET VALOR_A_PAGAR=:VALOR_A_PAGAR, VALOR_PAGO=:VALOR_PAGO,           '+
...
procedure TfrmEmprestimosPagar.txt_data_iEnter(Sender: TObject);
begin
  txt_data_i.Color :=$00F5EEDE;
end;

procedure TfrmEmprestimosPagar.txt_data_iExit(Sender: TObject);
begin
  txt_data_i.Color := clwindow;
end;

procedure TfrmEmprestimosPagar.txt_data_iKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    txt_Data_f.SetFocus;
...
procedure TfrmEmprestimosPagar.txt_data_fEnter(Sender: TObject);
begin
  txt_Data_f.Color :=$00F5EEDE;
end;

procedure TfrmEmprestimosPagar.txt_data_fExit(Sender: TObject);
begin
  txt_data_f.Color := clwindow;
end;

procedure TfrmEmprestimosPagar.txt_data_fKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
...
procedure TfrmEmprestimosPagar.btnPesquisarClick(Sender: TObject);
var
...
begin
  ...
  qryTotal.SQL.Add('SELECT SUM(VALOR_A_PAGAR) AS TOTAL_A_PAGAR,                             '+
                   ...
                   ' SUM(SALDO_A_PAGAR) AS TOTAL_SALDO_A_PAGAR FROM EMPRESTIMOS_A_PAGAR EP  '+
                   ' INNER JOIN TIPO_PAGAMENTO TP ON EP.TP_CODIGO = TP.TP_CODIGO            '+
...
procedure TfrmEmprestimosPagar.FormShow(Sender: TObject);
begin
  txt_data.SetFocus;
...
procedure TfrmEmprestimosPagar.DBGrid1DrawColumnCell(Sender: TObject;
...
procedure TfrmEmprestimosPagar.DBGrid1DblClick(Sender: TObject);
begin
...
procedure TfrmEmprestimosPagar.Receber1Click(Sender: TObject);
begin

  Application.CreateForm(TfrmPagarEmprestimo, frmPagarEmprestimo);

  frmPagarEmprestimo.txt_codigo.Text :=  qryEmprestimo.fieldbyname('CODIGO').AsString;

  frmPagarEmprestimo.qryRecebido.Close;
  frmPagarEmprestimo.qryRecebido.SQL.Clear;
  frmPagarEmprestimo.qryRecebido.SQL.Add('SELECT EP.*, EP.TP_CODIGO ||'' - ''|| TP.TP_DESCRICAO AS DESCRICAO FROM PAGTO_EMPRESTIMOS_A_PAGAR EP '+
                                         ' INNER JOIN TIPO_PAGAMENTO TP ON EP.TP_CODIGO = TP.TP_CODIGO                                         '+
                                         ' WHERE CODIGO=:CODIGO ORDER BY PG_CODIGO                                                             ');
  frmPagarEmprestimo.qryRecebido.ParamByName('CODIGO').AsString := frmPagarEmprestimo.txt_codigo.Text;
  frmPagarEmprestimo.qryRecebido.Open;

  frmPagarEmprestimo.qryTotal.Close;
  frmPagarEmprestimo.qryTotal.SQL.Clear;
  frmPagarEmprestimo.qryTotal.SQL.Add('SELECT SUM(PG_VALOR) AS TOTAL FROM PAGTO_EMPRESTIMOS_A_PAGAR WHERE CODIGO=:CODIGO');
  frmPagarEmprestimo.qryTotal.ParamByName('CODIGO').AsString := frmPagarEmprestimo.txt_codigo.Text;
  frmPagarEmprestimo.qryTotal.Open;

  frmPagarEmprestimo.txt_total_recebido.Value := frmPagarEmprestimo.qryTotal.fieldbyname('TOTAL').AsFloat;

  frmPagarEmprestimo.show;

end;

procedure TfrmEmprestimosPagar.TXT_OBS_pesqEnter(Sender: TObject);
begin
  txt_obs_pesq.Color := $00F5EEDE;
end;

procedure TfrmEmprestimosPagar.TXT_OBS_pesqExit(Sender: TObject);
begin
  txt_obs_pesq.Color := clwindow;
end;

procedure TfrmEmprestimosPagar.TXT_OBS_pesqKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
...
procedure TfrmEmprestimosPagar.cboFormaPgtoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    txt_obs.SetFocus;
...
procedure TfrmEmprestimosPagar.cboFormaPgtoEnter(Sender: TObject);
begin
  cboFormaPgto.color := $00F5EEDE;
end;

procedure TfrmEmprestimosPagar.cboFormaPgtoExit(Sender: TObject);
begin
  cboFormapgto.color := clwindow;
end;

procedure TfrmEmprestimosPagar.btn1Click(Sender: TObject);
begin
  Application.CreateForm(TfrmPagarEmprestimo, frmPagarEmprestimo);
  frmPagarEmprestimo.show;
end;

procedure TfrmEmprestimosPagar.DBGrid1TitleClick(Column: TColumn);
var
...
procedure TfrmEmprestimosPagar.Button1Click(Sender: TObject);
begin
