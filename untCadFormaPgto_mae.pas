...
type
  TfrmCadFormaPgto_mae = class(TForm)
    gpb_dados: TGroupBox;
    Label1: TLabel;
    txt_codigo: TEdit;
    Label2: TLabel;
...
var
  frmCadFormaPgto_mae: TfrmCadFormaPgto_mae;
  mensagem : string;
...
procedure TfrmCadFormaPgto_mae.limpa_campos(Sender: TObject);
begin
  txt_codigo.Clear;
  txt_descricao.Clear;
...
procedure TfrmCadFormaPgto_mae.txt_codigoEnter(Sender: TObject);
begin
  txt_codigo.Color := $00F5EEDE;
end;

procedure TfrmCadFormaPgto_mae.txt_codigoExit(Sender: TObject);
begin
  txt_codigo.Color := clwindow;
end;

procedure TfrmCadFormaPgto_mae.txt_descricaoEnter(Sender: TObject);
begin
  txt_descricao.Color := $00F5EEDE;
end;

procedure TfrmCadFormaPgto_mae.txt_descricaoExit(Sender: TObject);
begin
  txt_descricao.Color := clwindow;
end;

procedure TfrmCadFormaPgto_mae.FormShow(Sender: TObject);
begin
  ...
  qryTipoPgto.SQL.Add('SELECT * FROM TIPO_PAGAMENTO_mae');
  qryTipoPgto.Open;
...
procedure TfrmCadFormaPgto_mae.btn_inserirClick(Sender: TObject);
begin
  btn_inserir.Tag := 1;
...
procedure TfrmCadFormaPgto_mae.txt_descricaoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key =#13 then
    btn_gravar.SetFocus;
...
procedure TfrmCadFormaPgto_mae.btn_gravarClick(Sender: TObject);
var
  cod_forma : integer;
...
begin
  ...
  begin
    ...
    qry_trans_local.SQL.Add('UPDATE TIPO_PAGAMENTO_MAE SET TP_DEFAULT=:P_DEFAULT '+
...
//////////FIM VERIFICA SE DEFAULT ESTA MARCADO/////////
  ...
  qry_verifica.SQL.Add('SELECT MAX(TP_CODIGO) AS CODIGO FROM TIPO_PAGAMENTO_MAE');
  qry_verifica.Open;
  ...
  begin
    ...
    qry_trans_local.Close;
    qry_trans_local.SQL.Clear;
    qry_trans_local.SQL.Add('INSERT INTO TIPO_PAGAMENTO_MAE (TP_CODIGO, TP_DESCRICAO, TP_DEFAULT)'+
  ...
  begin
    ...
    qry_trans_local.Close;
    qry_trans_local.SQL.Clear;
    qry_trans_local.SQL.Add('UPDATE TIPO_PAGAMENTO_MAE SET TP_DESCRICAO=:P_DESCRICAO, TP_DEFAULT=:P_DEFAULT '+
...
procedure TfrmCadFormaPgto_mae.btn_alterarClick(Sender: TObject);
begin
  btn_inserir.Enabled := false;
  btn_gravar.Enabled := true;
  btn_cancelar.Enabled := true;
...
procedure TfrmCadFormaPgto_mae.btn_sairClick(Sender: TObject);
begin
  close;
end;

procedure TfrmCadFormaPgto_mae.btn_cancelarClick(Sender: TObject);
begin
  limpa_campos(Sender as TObject);
...
procedure TfrmCadFormaPgto_mae.btn_excluirClick(Sender: TObject);
begin
  ...
  begin
    ...
    qry_trans_local.Close;
    qry_trans_local.SQL.Clear;
    qry_trans_local.SQL.Add('delete from TIPO_PAGAMENTO_MAE where TP_codigo = ' + txt_codigo.Text);
    qry_trans_local.ExecSQL;
...
procedure TfrmCadFormaPgto_mae.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  frmCadFormaPgto_mae:= nil;
  action:= cafree;
end;

procedure TfrmCadFormaPgto_mae.DBGrid1DblClick(Sender: TObject);
begin
...
procedure TfrmCadFormaPgto_mae.DBGrid1DrawColumnCell(Sender: TObject;
